...
class BeerOrderStatusChangeEventListenerTest {
    ...
    void listen() {
        ...
        BeerOrder beerOrder = BeerOrder.builder().orderStatusEnum(OrderStatusEnum.READY)
                .orderStatusCallbackUrl("http://localhost:" + wireMockServer.port() + "/update")
                .createdDate(Timestamp.valueOf(LocalDateTime.now()))
                .build();

        BeerOrderStatusEvent event = new BeerOrderStatusEvent(beerOrder, guru.springframework.brewery.domain.OrderStatusEnum.NEW);
